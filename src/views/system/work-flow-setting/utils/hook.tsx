...
export function useHook() {
  ...
  async function onDel(row?: EnquiryGroupPageParam) {
    ...
    const res = await api.api.post_System_DelContractTypeByID(toRaw(row));
    if (res.Code === 0) {
      chores();
    } else {
  ...
  async function onSearch() {
    loading.value = true;
    ////GetWorkflowConfig
    const { Code, Data } = await api.api.post_System_GetWorkflowConfig({
  ...
  function openDialog(title = "新增", row?: UpdateAccountDTO) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        const curData = options.props.formInline as WorkflowConfig;
        function chores(res2: any) {
          if (res2.Code !== 0) {
            message(`操作失败，${res2.Message}`, { type: "error" });
            return;
          }
          message(`您${title}了任务名称为${curData.TypeName}的这条数据`, {
            type: "success"
          });
          done(); // 关闭弹框
          onSearch(); // 刷新表格数据
        }
        // 表单规则校验通过  ///////UpdateWorkflowConfig
        api.api.post_System_UpdateWorkflowConfig(toRaw(curData)).then(res => {
          // 实际开发先调用修改接口，再进行下面操作
          chores(res);
        });
      }
