...
export function useHook() {
  ...
  async function onSearch() {
    loading.value = true;
    ////account1/GetContractTypePage
    const { Code, Data } = await api.api.get_System_GetContractTypePage({
  ...
  function openDialog(title = "新增", row?: UpdateAccountDTO) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        // 表单规则校验通过  ///////UpdateContractType
        api.api.post_System_UpdateContractType(toRaw(curData)).then(res => {
          // 实际开发先调用修改接口，再进行下面操作
          chores(res);
        });
      }
