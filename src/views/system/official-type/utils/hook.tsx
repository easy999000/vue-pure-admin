...
export function useHook() {
  ...
  async function onDel(row?: EnquiryGroupPageParam) {
    ...
    const res = await api.api.post_System_DelOfficialTypeByID(toRaw(row));
    if (res.Code === 0) {
      chores();
    } else {
  ...
  async function onSearch() {
    loading.value = true;
    ////GetOfficialTypePage
    const { Code, Data } = await api.api.get_System_GetOfficialTypePage({
  ...
  function openDialog(title = "新增", row?: Officialtype) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        const curData = options.props.formInline as Officialtype;
        ...
        // 表单规则校验通过  ///////UpdateOfficialType
        api.api.post_System_UpdateOfficialType(toRaw(curData)).then(res => {
          // 实际开发先调用修改接口，再进行下面操作
          chores(res);
        });
      }
